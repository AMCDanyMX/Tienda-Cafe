...
//Daniel Alejandro Morales Castillo 3A 
//Desarrollo web
//tienda de cafesillo

//	Hice una tienda de venta de cage usando html, js y css
//	En el codigo javascript hay que hacer la base de datos de los productos, use un vector 

//BASE DE DATOS
	var productos = ["Cafe capuccino-4oz", "Cafe Americano-4oz", "Cafe expresso-4oz", "Cafe Latte-4oz","Cafe chico-8oz","Cafe mediano-12oz" ,"Cafe Grande - 16oz","Shots", "Pan"];
	var imgGrandes = ["img/productos/plantilla.jpg", "img/productos/2.jpg", "img/productos/3.jpg", "img/productos/4.jpg", "img/productos/5.jpg", "img/productos/6.jpg", "img/productos/7.jpg", "img/productos/8.jpg", "img/productos/9.jpg"];
	var imgPeque = ["img/productos/cafecapuccino.jpg", "img/productos/cafeamericano.jpg", "img/productos/expresso.jpg", "img/productos/latte.jpg", "img/productos/chico.jpg", "img/productos/mediano-grande.jpg", "img/productos/mediano-grande.jpg", "img/productos/shots.jpg", "img/productos/pan.jpg"];
	var precios = [1.00,1.00,1.00 ,1.55, 1.55,1.95 ,2.35 ,0.70 , 4];
	var stock = [100, 100, 100, 100, 100, 100, 100, 100, 200];
	var precioTransporte = [1, 2, 5, "Envio gratis"];
	var IVA = 0.18;
	var uniUser;
...
//JAVASCRIPT A EJECUTARSE UNA VEZ CARGADA LA PAGINA:
	window.onload = function(){
		...
		for (i in productos){
			DIVS[i].innerHTML = '<a id="imgG'+i+'" href="' +imgGrandes[i]+ '"><img id="imgP'+i+'" class="imagen" src="' +imgPeque[i]+ '"></a><div class="etiquetas"><b><span id="pro'+i+'">' +productos[i]+ '</span>:&nbsp;<span id="pre'+i+'">' + '</span> $ ' + precios[i]+ '<br/>  Seleccionar cantidad : <input class="uniBien" type="number" id="uniUser'+ i+'" name="uniUser" value="0" size="4" /></div>';
		}
...
/**/			document.getElementById("divPago").className = "divsNo";				
		...
		var precioUberEatsAPagar;
		if (numProductos <= 2){
			precioUberEatsAPagar = precioTransporte[0];
		}
		else if (numProductos <= 3){
			precioUberEatsAPagar = precioTransporte[1];
		}
		else if (numProductos <= 4){
			precioUberEatsAPagar = precioTransporte[2];
		}
		else if (numProductos >= 5){
			precioUberEatsAPagar = precioTransporte[3];
		}

		//Se sacan las cuentas del transporte (si lo hubiese), del iva y el total:
		var totalTransporte = precioUberEatsAPagar;
